...
This GSE contain both scRNA-seq and bulk RNA-seq sample. Removal of bulk RNA-seq is needed to avoid significant batch effect due to their technical difference. Although the number of sample is reduced to 12, it's still sufficient.
...
keep_platform <- "GPL24676"    # The scRNA-seq platform (NovaSeq)
...
sql_filter <- paste0(
  ...
  "AND t2.gpl = '", keep_platform, "'"
)

# 3. Get the list of pure scRNA-seq samples
sc_gsm_list <- dbGetQuery(con, sql_filter)

# 4. View results
print(paste("Found", nrow(sc_gsm_list), "scRNA-seq samples."))
head(sc_gsm_list)
```
